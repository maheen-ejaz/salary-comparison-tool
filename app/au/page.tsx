...
import { AuPageClient } from "./AuPageClient";
...
export default async function AuPage() {
  ...
  let auRate = { rate: 0, isLive: false, date: "" };
  for (const r of rateResults) {
    allRates[r.code] = r.rate;
    if (r.code === "au") {
      auRate = { rate: r.rate, isLive: r.isLive, date: r.date };
    }
  }

  return (
    <AuPageClient
      data={allCountryData["au"]}
      config={config}
      liveRate={auRate.rate}
      rateIsLive={auRate.isLive}
      rateDate={auRate.date}
